...
- (void)addFriendById:(NSString*)userName
{
    ...
    userName = [NSString stringWithFormat:@"%@@%@",userName,kXMPP_HOST];
    [self.xmppRoster addUser:[XMPPJID jidWithString:userName] withNickname:@"好友"];
}
...
- (void)goOnline
{
    ...
    [presence addChild:[DDXMLNode elementWithName:@"status" stringValue:@"我在线上"]];
...
{
    NSLog(@"%s",__func__);
//    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"您的帐号在另地登录，被迫下线！" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil,nil];
//    alert.tag = 1039;
//    [alert show];
}
...
{
    MyLog(@"%s--%@",__FUNCTION__, message);
    //XEP--0136 已经用coreData实现了数据的接收和保存
    //收到消息
    [[NSNotificationCenter defaultCenter] postNotificationName:XMPP_MESSAGE_RECEIVE object:message];
}

#pragma mark - UIAlertViewDelegate
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 1039) {
